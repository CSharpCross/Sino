...
namespace OrvilleX.EventBus.Common
{
    ...
    public class TopologyProvider : ITopologyProvider
    {
        ...
        {
            return queue.AssumeInitialized || _initQueues.Contains(queue.FullQueueName);
        }
        ...
        private void DeclareQueue(QueueConfiguration queue)
        {
            ...
            _logger.LogInformation($"声明队列'{queue.FullQueueName}'。");
        ...
        private void DeclareExchange(ExchangeConfiguration exchange)
        {
            ...
            _logger.LogInformation($"声明交换器'{exchange.ExchangeName}'。");
            var channel = GetOrCreateChannel();
        ...
        private void EnsureWorker()
        {
            ...
            while (_topologyTasks.TryDequeue(out topologyTask))
            {
                ...
                if (exchange != null)
                {
                    ...
                    {
                        _logger.LogError(1, e, $"无法声明交换器'{exchange.Configuration.ExchangeName}'");
                ...
                if (queue != null)
                {
                    ...
                    {
                        _logger.LogError(1, e, $"无法声明队列'{queue.Configuration.QueueName}'");
